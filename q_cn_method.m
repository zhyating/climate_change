function Y = q_cn_method(x)   %(Ts, T_film, rho_f, kf, q_cn, q_r, R, mu_f)
Vw = x(9);
Ta = x(10);
N = x(11);
Hour = x(12); 
...
I = (0.6 + (Ta - 30) * 0.09) * Ampacity;

Y(1,1) = (Ta + x(1)) / 2 - x(2);
Y(2,1) = (1.458/10^6 * (x(2) + 273)^1.5) / (x(2) + 383.4) - x(8);
...
Y(5,1) = 3.645 * x(3)^0.5 * D0^0.75 * (x(1) - Ta)^1.25 - x(5);
Y(6,1) = 17.8 * D0 * E * (((x(1) + 273)/100)^4 - ((Ta + 273)/100)^4) - x(6);               %Radiated heat loss rate, w/m
Y(7,1) = (x(5) + x(6) - q_s) / I^2 - x(7); 
Y(8,1) = (x(7) - R_25) * (75 - 25) / (R_75 - R_25) + 25 - x(1);
